  ...
  return (
    <div
      className={cn(
        ...
        isMini ? 'items-center border-gray-300/70 bg-white' : 'items-end gap-2 rounded-2xl p-2',
      )}
      data-no-pan=""
    ...
    >
      <button
        ...
        disabled={disabled}
        aria-label={speechEnabled ? 'Disable spoken responses' : 'Enable spoken responses'}
        aria-pressed={speechEnabled}
      >
        {isSpeaking ? <Loader2 className={cn(iconClass, 'animate-spin')} /> : <Speech className={iconClass} />}
      </button>
      ...
      >
        {isTranscribing ? <Loader2 className={cn(iconClass, 'animate-spin')} /> : <Mic className={iconClass} />}
      </button>
