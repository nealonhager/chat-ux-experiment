...
function ActiveNodeViewportSync({
  ...
  useEffect(() => {
    ...
    const activeBubble = bubbles.find((bubble) => bubble.id === tree.activeNodeId)
    ...
    if (!isVisible) {
      ...
      panToWorldPoint(activeBubble.x + activeBubble.width / 2, activeBubble.y + activeBubble.height / 2)
...
export function ChatBubbles({
  ...
  const threadSegments = useMemo(() => getBubbleThreadSegments(bubbles), [bubbles])
  ...
  return (
    ...
    <>
      ...
      <ActiveNodeViewportSync tree={tree} thinkingParentId={thinkingParentId} isSending={isSending} />
      ...
      >
        ...
        <svg aria-hidden className="pointer-events-none absolute inset-0" width={WORLD_SIZE} height={WORLD_SIZE}>
