...
export function PanZoomLayer({
  ...
  const panStartRef = useRef({ pointerX: 0, pointerY: 0, transformX: 0, transformY: 0 })

  transformRef.current = transform

  const updateTransform = useCallback((next: PanZoomTransform, viewport = getViewportSize()) => {
  ...
  const panToWorldPoint = useCallback(
    (worldX: number, worldY: number) => {
      updateTransform({
        ...
        ...panToWorldCenter(worldX, worldY, transformRef.current.scale, viewportSize),
      })
    },
  ...
  useEffect(() => {
    function handlePointerMove(event: PointerEvent): void {
      ...
      const { pointerX, pointerY, transformX, transformY } = panStartRef.current
      updateTransform({
        ...transformRef.current,
  ...
  function handleWheel(event: React.WheelEvent<HTMLDivElement>): void {
    ...
    updateTransform(zoomAtPoint(transformRef.current, event.clientX, event.clientY, deltaScale))
  }
      ...
      >
        <div className="origin-top-left" style={toPanStyle(transform)}>
          <div
            ...
            style={{ ...toZoomStyle(transform), width: WORLD_SIZE, height: WORLD_SIZE }}
          >
            {children}
          </div>
        </div>
