...
export function getCenteredPan(scale: number, viewportWidth = window.innerWidth): Pick<PanZoomTransform, 'x' | 'y'> {
  return clampPan(
    viewportWidth / 2 - (WORLD_SIZE / 2) * scale,
    0,
    scale,
    { width: viewportWidth, height: window.innerHeight },
  )
}
...
export function clampTransform(
  ...
  const { x, y } = clampPan(transform.x, transform.y, transform.scale, viewport)
...
export function worldRectToMinimap(rect: WorldRect, minimapSize = MINIMAP_SIZE): WorldRect {
