...
export class RealtimeTranscriptionSession {
  ...
  private handleEvent(rawEvent: MessageEvent<string>): void {
    ...
    if (event.type === 'conversation.item.input_audio_transcription.completed') {
      const itemId = event.item_id ?? 'default'
      const transcript = (event.transcript ?? this.drafts.get(itemId) ?? '').trim()
    ...
    if (event.type === 'error') {
      this.handlers.onError(event.error?.message ?? 'Realtime transcription error.')
    }
  }
  ...
  async start(): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error((await response.text()) || 'Could not start transcription session.')
    }
