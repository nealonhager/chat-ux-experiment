...
function estimateBubbleHeight(
  ...
  const charsPerLine = Math.max(20, Math.floor(getBubbleContentWidth(bubbleWidth) / 6.5))
  const lines = Math.max(1, Math.ceil(content.length / charsPerLine))
  const textHeight = lines * CHAT_LAYOUT.lineHeight
  let height = CHAT_LAYOUT.bubblePaddingY * 2 + textHeight + CHAT_LAYOUT.chromeSlack
...
function buildLayoutForest(tree: ConversationTree): LayoutNode[] {
  function buildNode(message: ChatMessage): LayoutNode {
    const childMessages = getChildren(tree, message.id).sort((a, b) => a.id.localeCompare(b.id))
    return {
      message,
...
function assignPositions(nodes: LayoutNode[], bubbleWidth: number, startY: number, startX: number): number {
  let cursorX = startX

  function layoutNode(node: LayoutNode, parentBottom: number): number {
    node.height = estimateBubbleHeight(node.message.content, bubbleWidth, node.message.role)
...
export function getBubbleWorldRectsFromTree(
  ...
  assignPositions(forest, bubbleWidth, CHAT_LAYOUT.paddingTop, (WORLD_SIZE - CHAT_COLUMN_WIDTH) / 2 + CHAT_LAYOUT.paddingX)
  ...
  if (isSending) {
    ...
    const height = estimateBubbleHeight('Thinking...', bubbleWidth, 'assistant')
